...
for level in range(1, num_levels + 1):
    ...
    with col3:
        ...
        increment_percent = st.number_input(f"L{level} Increment %", value=1.0 * level, step=0.1, format="%.2f", 
                                          ...
                                          help="% of coin-in that goes to this level")
        increment_ratio = increment_percent / 100
        
        if jackpot_type == "Mystery Progressive":
            st.metric("Daily Increment", f"€{increment_ratio * total_coin_in:,.2f}")
...
if not error_flag and total_coin_in > 0:
    ...
    if jackpot_type == "Mystery Progressive":
        ...
        df["JP Increment per Day (€)"] = df["Increment Ratio"] * total_coin_in
        
        # Calculate time to reach max
        df["Days to Must Hit"] = df["Hit Range Size (€)"] / df["JP Increment per Day (€)"]
        ...
        df["Estimated JP Paid / Month (€)"] = df["Hits per Month"] * df["Avg Hit Value (€)"]
        df["Real RTP (%)"] = (df["Estimated JP Paid / Day (€)"] / total_coin_in) * 100
        ...
        df["Real RTP (%)"] = ((df["Avg Hit Value (€)"] * df["Hits per Day"]) / total_coin_in) * 100
        df["JP Increment per Day (€)"] = df["Increment Ratio"] * total_coin_in
        df["Estimated JP Paid / Day (€)"] = df["Hits per Day"] * df["Avg Hit Value (€)"]
    ...
    total_increment = df["JP Increment per Day (€)"].sum()
    total_rtp = df["Real RTP (%)"].sum()
    total_paid_day = df["Estimated JP Paid / Day (€)"].sum()
    total_paid_month = df["Estimated JP Paid / Month (€)"].sum()
    
    metric_col1, metric_col2, metric_col3, metric_col4 = st.columns(4)
    
    with metric_col1:
        st.metric("Total Increment/Day", f"€{total_increment:,.2f}")
    
    with metric_col2:
        st.metric("Total RTP %", f"{total_rtp:.2f}%")
    
    with metric_col3:
        st.metric("Est. Paid/Day", f"€{total_paid_day:,.2f}")
    
    with metric_col4:
        st.metric("Est. Paid/Month", f"€{total_paid_month:,.2f}")
    ...
    if jackpot_type == "Mystery Progressive":
        display_cols = ["Level", "Start Value", "Max Value", "Avg Hit Value (€)", 
                       "Increment %", "JP Increment per Day (€)", "Days to Must Hit",
                       "Hits per Month", "Estimated JP Paid / Month (€)", "Real RTP (%)"]
    else:
        display_cols = ["Level", "Start Value", "Max Value", "Trigger Value", "Avg Hit Value (€)",
                       "Increment %", "JP Increment per Day (€)", "Hits per Day", "Avg Days per Hit",
                       "Estimated JP Paid / Month (€)", "Real RTP (%)"]
    ...
    for idx, row in df.iterrows():
        ...
        if jackpot_type == "Mystery Progressive":
            ...
            **Level {level} ({jackpot_type})**:
            ...
            - 📊 Contributes {row['Real RTP (%)']:.2f}% to RTP
            - 💰 Expected {row['Hits per Month']:.1f} hits/month averaging €{row['Avg Hit Value (€)']:,.2f}
            - 📈 Grows by €{row['JP Increment per Day (€)']:,.2f}/day ({row['Increment %']:.2f}% of coin-in)
            """)
        else:
            st.info(f"""
            **Level {level} ({jackpot_type})**:
