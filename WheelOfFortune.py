...
if valid_input:
    ...
    if run_customer_sim:
        ...
        if use_promo_ticket:
            ...
            avg_customer_eur = avg_customer_points
        st.success(f"Average for {num_trials:,} customers spinning {num_spins}x: **{avg_customer_points:,.2f} value ({'after survival' if use_promo_ticket else 'points'}) = €{avg_customer_eur:,.2f}**")
        st.write(f"- Min: {np.min(total_points_list):,.0f} value, Max: {np.max(total_points_list):,.0f} value")
    ...
    with chart_cols[1]:
        ...
        ax_bar.set_ylabel("Value per Compartment", fontsize=10)
