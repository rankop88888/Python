...
st.set_page_config(page_title="Wheel of Fortune Simulator", layout="wide")
st.title("🎡 Wheel of Fortune Promo Simulator")

st.info("Configure your wheel, run simulations, and estimate your real promo expense per spin, customer, or campaign!")
...
# Expand or trim defaults to match num_compartments
...
st.subheader("Wheel Expected Values")
st.write(f"- **Average points per spin:** {avg_points:,.2f}")
st.write(f"- **Average cost per spin:** €{avg_wheel_cost:,.2f}")
st.write(f"- **Average cost for {num_spins} spins (one customer):** €{expected_per_customer:,.2f}")
st.write(f"- **Total promo cost per day (all customers, all spins):** €{total_daily_cost:,.2f}")

# --- 4. SIMULATE CUSTOMER SPINNING X TIMES IN A ROW ---
st.header("3. Simulate Repeated Spins for a Single Customer")
...
st.header("4. Wheel Distribution Visualization")
...
# --- 6. IF ALSO GIVING PROMO TICKET WITH WAGER ---
st.header("5. Combine with Promo Ticket Giveaway")
give_promo = st.checkbox("Also give a Promo Ticket?")

if give_promo:
    st.markdown("*Estimate the average extra cost if every customer also gets a promo ticket with wagering requirement.*")
    ...
    st.write(f"- **Expected cost per wheel+promo spin:** €{combined_cost:,.2f} (Wheel: €{avg_wheel_cost:,.2f} + Promo Ticket: €{promo_exp_cost:,.2f})")
    st.write(f"- **Per customer ({num_spins} spins, 1 ticket):** €{num_spins*combined_cost:,.2f}")
    st.write(f"- **Total daily cost (all customers, all spins, all tickets):** €{num_customers*sets_per_day*num_spins*combined_cost:,.2f}")
...
if give_promo:
    ...
    summary_dict["Combined Total Cost per Day (€)"] = [num_customers*sets_per_day*num_spins*combined_cost]
...
st.caption("💡 Tip: You can model VIP/Regular by adjusting points, spins, or customer numbers. Combine with your promo ticket simulator for full campaign cost analytics!")
